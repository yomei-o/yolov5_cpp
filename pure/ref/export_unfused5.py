...
qn = {id(mm): nm for nm, mm in m.model.named_modules()}   # module -> state_dict prefix (model.N...)

def save(n, t): t.detach().contiguous().float().cpu().numpy().tofile(os.path.join(D, n))
lines = [str(len(mods))]
names = []            # state_dict KEY per emitted tensor in engine order (C3 emit order
                      # differs from state_dict order, so the C++ checkpointer pairs by name)
