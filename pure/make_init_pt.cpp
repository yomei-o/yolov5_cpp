...
// state_dict .pt via the pure-C++ writer. The result loads in the yolov5 reference model
// (load_state_dict) and is trainable by train_cli.
...
int main(int argc, char** argv) {
  ...
  if (!mf) { printf("cannot open %smanifest_unfused.txt (run export_unfused5.py once)\n", DN.c_str()); return 1; }
  ...
  // nothing (a raw {'model': nn.Module} checkpoint), walk the module tree.
  ...
  for (int i = 0; i < nlayers; ++i) {
    int kind; int64_t Co, Ci, k, s, pad; double eps; mf >> kind >> Co >> Ci >> k >> s >> pad >> eps;
