...
# Device-resident files (dtensor.hpp / dnet5.hpp / thrust) need CUDA CCCL headers; skipped
# unless THRUST is set, e.g.
#   THRUST="-std=c++17 -I/usr/local/cuda/include/cccl -DTHRUST_DEVICE_SYSTEM=THRUST_DEVICE_SYSTEM_CPP" ./run_all.sh
#   CXX="nvcc -x cu -DUSE_CUDA" THRUST="--extended-lambda" ./run_all.sh
echo "== compile every pure/*.cpp with: $CXX =="
for f in pure/*.cpp; do
  if grep -qE 'thrust/|dtensor\.hpp|dnet5\.hpp' "$f"; then
    if [ -n "${THRUST:-}" ]; then
